...
class SpringDataLabApplicationRepositoryTest extends BaseRepositoryTest {
    ...
    void save_generatesId() {
        // given
        Lab lab = IntegrationLabFactory.persistValidLab(entityManager);
        User user = IntegrationUserFactory.persistValidUser(entityManager);
        LocalDateTime time = LocalDateTime.now().plusDays(1);
        // when
        LabApplication saved = IntegrationLabApplicationFactory.persistValidPendingApplication(entityManager, lab, user, time);
        // then
    ...
    void findByLabId_returnsOnlyThatLabApplications() {
        ...
        IntegrationLabApplicationFactory.persistValidPendingApplication(entityManager, lab1, user1, t1);
        IntegrationLabApplicationFactory.persistValidPendingApplication(entityManager, lab1, user2, t2);
        IntegrationLabApplicationFactory.persistValidPendingApplication(entityManager, lab2, user1, t1);
    ...
    void existsByLabIdAndUser_existing_returnsTrue() {
        Lab lab = IntegrationLabFactory.persistValidLab(entityManager);
        User user = IntegrationUserFactory.persistValidUser(entityManager);
        IntegrationLabApplicationFactory.persistValidPendingApplication(entityManager, lab, user, LocalDateTime.now().plusDays(1));
    ...
    void delete_persistsRemoval() {
        Lab lab = IntegrationLabFactory.persistValidLab(entityManager);
        User user = IntegrationUserFactory.persistValidUser(entityManager);
        LabApplication saved = IntegrationLabApplicationFactory.persistValidPendingApplication(entityManager, lab, user, LocalDateTime.now().plusDays(1));
