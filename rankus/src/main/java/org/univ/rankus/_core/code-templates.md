...
public class {Domain}CommandService implements {Domain}CommandUseCase {
    ...
    public {Domain} create{Domain}({Domain}CreateRequestDto request) {
        // 1. 검증
        // 2. 도메인 생성
        {Domain} {domain} = {Domain}.create(request.getName());
        // 3. 저장
    ...
    @Override
    @Transactional
    public {Domain} update{Domain}(Long id, {Domain}UpdateRequestDto request) {
        // 1. 조회
        {Domain} {domain} = {domain}RepositoryPort.findById(id)
            .orElseThrow(() -> new {Domain}NotFoundException(id));
        // 2. 업데이트
        {domain}.update{Field}(request.getName());
        // 3. 저장
        return {domain}RepositoryPort.save({domain});
    }
    ...
    public void delete{Domain}(Long id) {
        {Domain} {domain} = {domain}RepositoryPort.findById(id)
            .orElseThrow(() -> new {Domain}NotFoundException(id));
        {domain}RepositoryPort.delete({domain});
    }
}
```
...
public class {Domain}Controller {
    ...
    public ResponseEntity<ApiResponse<List<{Domain}ResponseDto>>> get{Domain}List() {
        List<{Domain}> {domains} = {domain}QueryUseCase.findAll{Domain}s();
...
public record {Domain}UpdateRequestDto(
    @NotBlank(message = "{필드}는 필수입니다")
    @Size(max = 100, message = "{필드}는 100자 이하여야 합니다")
    String {field}
) {}
